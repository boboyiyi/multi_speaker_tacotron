...
def encoder_cbhg(inputs, input_lengths, is_training, before_highway=None, encoder_rnn_init_state=None):
  return cbhg(
    inputs=inputs,
...
def cbhg(inputs, input_lengths, is_training, scope, K, projections, before_highway=None, encoder_rnn_init_state=None):
  with tf.variable_scope(scope):
    ...
    if before_highway is not None:
      ...
      tiled_before_highway = tf.tile(expanded_before_highway, [1, tf.shape(proj2_output)[1], 1])
      highway_input = proj2_output + inputs + tiled_before_highway
    else:
