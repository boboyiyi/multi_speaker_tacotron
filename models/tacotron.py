...
class Tacotron():
  ...
  def initialize(self, inputs, input_lengths, target_lengths, prefixes=None, speaker_ids=None, mel_targets=None, linear_targets=None):
    ...
    with tf.variable_scope('inference') as scope:
      ...
      speaker_embedding_table = tf.get_variable('speaker_embedding_table', [hp.num_speakers, hp.speaker_embedding_size], dtype=tf.float32, initializer=tf.truncated_normal_initializer(stddev=0.5))
      speaker_embedding = tf.nn.embedding_lookup(speaker_embedding_table, speaker_ids) # [N, T_in, hp.speaker_embedding_size]
      ...
      decoder_rnn_init_states = [deep_dense(speaker_embedding, 256) for _ in range(2)]
      # Encoder
      prenet_outputs = prenet(inputs, is_training)                                # [N, T_in, 128]
      encoder_outputs = encoder_cbhg(prenet_outputs, input_lengths, is_training, before_highway=before_highway, encoder_rnn_init_state=encoder_rnn_init_state)  # [N, T_in, 256]
